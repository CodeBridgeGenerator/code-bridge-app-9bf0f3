...
import seederData from "./trainingProvidersDescription.seeder.json";
...
const TrainingProvidersDescriptionSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("trainingProvidersDescription").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(TrainingProvidersDescriptionSeederDialogComponent);
