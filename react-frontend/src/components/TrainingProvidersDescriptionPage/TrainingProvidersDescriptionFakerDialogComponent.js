...
import trainingProvidersDescriptionFakerFactory from "./trainingProvidersDescriptionFakerFactory";
...
const TrainingProvidersDescriptionFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = trainingProvidersDescriptionFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainingProvidersDescription").create(elem));
...
export default connect(mapState, mapDispatch)(TrainingProvidersDescriptionFakerDialogComponent);
