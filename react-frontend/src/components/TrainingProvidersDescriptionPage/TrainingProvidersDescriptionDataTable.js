...
const TrainingProvidersDescriptionDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.organizationAddress}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "trainingProvidersDescription"});
...
<Column field="organizationAddress" header="Organization Address" body={pTemplate1} filter={selectedFilterFields.includes("organizationAddress")} hidden={selectedHideFields?.includes("organizationAddress")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload TrainingProvidersDescription Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="trainingProvidersDescription"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search TrainingProvidersDescription" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default TrainingProvidersDescriptionDataTable;