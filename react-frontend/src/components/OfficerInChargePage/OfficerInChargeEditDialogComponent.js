...
const OfficerInChargeCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            officerName: _entity?.officerName,
designation: _entity?.designation,
hp: _entity?.hp,
icPassportNumber: _entity?.icPassportNumber,
email: _entity?.email,
organizationName: _entity?.organizationName,
        ...
        const result = await client.service("officerInCharge").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info officerInCharge updated successfully" });
    ...
    return (
        <Dialog header="Edit Officer In Charge" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="officerInCharge-edit-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            ...
            <InputNumber id="hp" className="w-full mb-3 p-inputtext-sm" value={_entity?.hp} onChange={(e) => setValByKey("hp", e.value)}  />
        </span>
        </div>
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            <label htmlFor="icPassportNumber">IC / Passport Number:</label>
            <InputText id="icPassportNumber" className="w-full mb-3 p-inputtext-sm" value={_entity?.icPassportNumber} onChange={(e) => setValByKey("icPassportNumber", e.target.value)}  />
        </span>
        </div>
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
