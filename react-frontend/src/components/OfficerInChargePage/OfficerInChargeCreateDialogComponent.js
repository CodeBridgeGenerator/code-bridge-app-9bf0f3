...
const OfficerInChargeCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            officerName: _entity?.officerName,designation: _entity?.designation,hp: _entity?.hp,icPassportNumber: _entity?.icPassportNumber,email: _entity?.email,organizationName: _entity?.organizationName,
...
<div className="col-12 md:col-6 field mt-5">
            ...
            <small className="p-error">
            {!_.isEmpty(error["icPassportNumber"]) ? (
              <p className="m-0" key="error-icPassportNumber">
                {error["icPassportNumber"]}
              </p>
            ) : null}
          </small>
            </div>
